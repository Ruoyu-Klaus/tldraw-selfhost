...
### 9.2 前提：通过 Tailscale 直连到运行 tldraw 的机器

当前方案不再依赖 Cloudflare Access Service Token。

只要 Cursor 所在电脑已经接入同一个 Tailscale tailnet，并且能直接访问运行 tldraw 的那台机器，就可以让 MCP 直接连内网地址，绕过浏览器访问时才需要处理的 Cloudflare 认证。

准备条件：

1. **服务端机器已登录 Tailscale**  
   确认运行 `tldraw-selfhost` 的机器已经加入 tailnet。

2. **Cursor 所在机器也已登录同一个 tailnet**  
   两台机器之间应当可以互相访问。

3. **确认 tldraw 服务可从 tailnet 内访问**  
   例如服务跑在 `5858` 端口时，能在另一台机器上打开：
   - `http://100.x.x.x:5858`
   - 或 `http://your-machine.tailnet-name.ts.net:5858`

> 如果你平时对外是通过 Cloudflare Tunnel 打开的网页，这里也没关系。MCP 只要改成走 Tailscale 内网地址即可，不需要再传 `CF_ACCESS_CLIENT_ID` / `CF_ACCESS_CLIENT_SECRET`。
...
可选地，你也可以把 `TLDRAW_BASE_URL` 写成 Tailscale 的 MagicDNS 域名，例如：

```json
{
  "env": {
    "TLDRAW_BASE_URL": "http://mac-mini.your-tailnet.ts.net:5858",
    "MCP_TOKEN": "与服务端相同的 MCP_TOKEN"
  }
}
```

> **升级 MCP 时**：把 `args` 里 URL 中的版本号（`v1.0.0` / `1.0.0`）替换为新 Tag 版本，保存即可。其余 env 一般无需改动。
>
> **连接失败时优先检查**：
> 1. 两台机器是否都在线并已接入同一个 Tailscale tailnet
> 2. `TLDRAW_BASE_URL` 是否填写成了服务端机器的 Tailscale 地址，而不是 Cloudflare 域名
> 3. `MCP_TOKEN` 是否与服务端一致
> 4. 目标端口 `5858` 是否可达
